...
class AdvancedLinearConstraintInterface(AdvancedInterface):
    ...
    def add_lazy_cuts(self, lin_expr = [], senses = "", rhs = [], names = []):
        ...
        num_lazy_cuts = validate_arg_lengths(
            self._env, "linear_constraints.advanced.add_lazy_cuts",
    ...
    def add_user_cuts(self, lin_expr = [], senses = "", rhs = [], names = []):
        ...
        num_user_cuts = validate_arg_lengths(
            self._env, "linear_constraints.advanced.add_user_cuts",
            [rhs, senses, names, lin_expr])
        if isinstance(lin_expr, type([])):
            rmat = _HBMatrix(lin_expr)
...
class LinearConstraintInterface(IndexedInterface):
    ...
    def add(self, lin_expr = [], senses = "", rhs = [], range_values = [], names = []):
        ...
        num_new_rows = validate_arg_lengths(
            self._env, "linear_constraints.add",
        ...
        if lin_expr == []:
            ...
            CPX_PROC.addrows(self._env._e, self._cplex._lp, 0, num_new_rows,
                             ...
                             self._env.parameters.read.apiencoding.get())
            if range_values != []:
                CPX_PROC.chgrngval(self._env._e, self._cplex._lp, list(range(num_old_rows, num_old_rows + num_new_rows)), range_values)
    ...
    def set_linear_components(self, *args):
        ...
        def setlin(aa, bb):
            for i, a in enumerate(aa):
                ...
                if len(ind) != len(val):
                    raise CplexError(
                        "Inconsistent arguments to " +
                        "linear_constraints.set_linear_components")
...
class IndicatorConstraintInterface(IndexedInterface):
    ...
    def get_rhs(self, *args):
        ...
        """
        def getrhs(a):
            return CPX_PROC.getindconstr_constant(self._env._e,
                                                  self._cplex._lp, a)[3]
        return apply_freeform_one_arg("indicator_constraints.get_rhs", getrhs,
...
class QuadraticConstraintInterface(IndexedInterface):
    ...
    def add(self, lin_expr = SparsePair([0], [0.0]), quad_expr = SparseTriple([0], [0], [0.0]), sense = "L", rhs = 0.0, name = ""):
        ...
        if len(ind) != len(val):
            raise CplexError(
                "Inconsistent arguments to quadratic_constraints.add")
...
class MIPStartsInterface(IndexedInterface):
    ...
    def change(self, *args):
        ...
        if len(args) == 3:
            ...
            if len(ind) != len(val):
                raise CplexError(
                    "Inconsistent arguments to MIP_starts.change")
...
class MIPSolutionInterface(AdvancedInterface):
    ...
    def get_best_objective(self):
        """Returns the currently best known bound of all the remaining open
        nodes in a branch-and-cut tree.
...
class SolnPoolFilterInterface(AdvancedInterface):
    ...
    def get_indices(self, name):
        ...
        """
        if isinstance(name, six.string_types):
            return CPX_PROC.getsolnpoolfilterindex(
                self._env._e, self._cplex._lp, name,
                self._env.parameters.read.apiencoding.get())
        else:
            return [CPX_PROC.getsolnpoolfilterindex(
